...
func TestOperatorCrypto(t *testing.T) {
	t.Run("VerifyTaskRequest", func(t *testing.T) {
		...
		tts := []struct {
			...
			{
				name: "should call verifyTaskRequest and handler if the crypto is not nil and signature checking is enabled",
		...
		for _, tt := range tts {
			t.Run(tt.name, func(t *testing.T) {
				...
				client := respondertest.NewResponder()
				o, err := orbital.NewOperator(orbital.TargetOperator{Client: client, Signer: respSigner, Verifier: tt.reqVerifier, MustCheckSignature: tt.mustCheckSignature})
				assert.NoError(t, err)
				assert.NotNil(t, o)
