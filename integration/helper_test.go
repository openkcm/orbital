...
func createAndStartManager(ctx context.Context, t *testing.T, store *sql.SQL, config managerConfig) (*orbital.Manager, error) {
	...
	managerOpts := []orbital.ManagerOptsFunc{
		...
		orbital.WithTargetInitiators(config.targetInitiators),
...
func createAndStartOperator(ctx context.Context, t *testing.T, client orbital.ResponderClient, config operatorConfig) error {
	t.Helper()

	operator, err := orbital.NewOperator(orbital.Responder{Client: client})
	if err != nil {
